...
export class SkygearChatContainer {
  ...
  _getMessageIDsByConversations(conversations) {
    return _.reduce(conversations, function (mids, conversation) {
      if (conversation.last_message_ref) {
        const mid = skygear.Record.parseID(conversation.last_message_ref.id)[1];
        mids.push(mid);
      }
      if (conversation.last_read_message_ref) {
        const last_read_message_id = skygear.Record.parseID(
                conversation.last_read_message_ref.id)[1];
  ...
  _getMessagesOfConversations(conversations) {
    var messageIDs = this._getMessageIDsByConversations(conversations);
    var messagesByID = this._getMessagesByIDs(messageIDs);
    return _.map(conversations,
            (conversation) => {
              if (conversation.last_message_ref) {
                conversation.last_message =
                    messagesByID[conversation.last_message_ref.id];
              }

              if (conversation.last_read_message_ref) {
                conversation.last_read_message =
                    messagesByID[conversation.last_read_message_ref];
              }
              return conversation;
            });
  }
  ...
  getConversation(conversationID, includeLastMessage = true) {
    ...
    query.equalTo('conversation', new skygear.Reference('conversation/' + conversationID));
    ...
    return skygear.publicDB.query(query).then((records) => {
      if (records.length > 0) {
        var uc = records[0];
        var conversation = Conversation.fromRecord(uc.$transient.conversation,
                uc.unread_count,
                uc.last_read_message);
        if (includeLastMessage) {
          return this._getMessagesOfConversations([conversation])[0];
        } else {
          return conversation;
        }
      }
  ...
  getConversations(page = 1, pageSize = 50, includeLastMessage = true) {
    ...
    return skygear.publicDB.query(query).then((records) => {
      var conversations = _.map(records, (record) => {
        return Conversation.fromRecord(record.$transient.conversation,
                record.unread_count,
                record.last_read_message);
      })
      if (includeLastMessage) {
        return this._getMessagesOfConversations(conversations);
      }
  ...
  getMessages(conversation, limit = 50, beforeTime) {
    const conversationID = conversatio.id;
    return skygear

...
export class SkygearChatContainer {
  ...
  async createConversation(
    ...
    const result = await skygear.lambda(
   ...
   * @return {Promise<Conversation>} - Promise of the new Conversation Record
   */
  async createDirectConversation(user, title = null, meta = {}, options = {}) {
   ...
   */
  async getConversation(conversationID, includeLastMessage = true) {
    const data = await skygear
   ...
   * @return {Promise<[]Conversation>} A promise to array of Conversation.
   */
  async getConversations(page = 1, pageSize = 50, includeLastMessage = true) {
    const data = await skygear
   ...
   * @private
   */
  async _getUserConversation(conversation) {
    ...
    const records = await skygear.publicDB.query(query);
    if (records.length > 0) {
      return records[0];
    }
    throw new Error('no conversation found');
   ...
   */
  async updateConversation(conversation, title, meta) {
    ...
    _.map(Conversation.RESTRICTED_FIELD_KEYS, (eachKey) => {
      delete newConversation[eachKey];
    });
  ...
  async leaveConversation(conversation) {
    await skygear.lambda('chat:leave_conversation', [conversation._id]);
    return true;
  }
  ...
  async deleteConversation(conversation) {
    await skygear.lambda('chat:delete_conversation', [conversation._id]);
    return true;
  }
   ...
   * @return {Promise<Conversation>} - A promise to save result
   */
  async addParticipants(conversation, participants) {
   ...
   * @return {Promise<COnversation>} - A promise to save result
   */
  async removeParticipants(conversation, participants) {
    const conversation_id = skygear.Record.parseID(conversation.id)[1];
    const participant_ids = _.map(participants, function (user) {
      return user._id;
    });
   ...
   * @return {Promise<Conversation>} - A promise to save result
   */
  async addAdmins(conversation, admins) {
    const conversation_id = skygear.Record.parseID(conversation.id)[1];
   ...
   * @return {Promise<Conversation>} - A promise to save result
   */
  async removeAdmins(conversation, admins) {
    const conversation_id = skygear.Record.parseID(conversation.id)[1];
    const admin_ids = _.map(admins, function (user) {
      return user._id;
    });
   ...
   */
  async createMessage(conversation, body, metadata, asset) {
   ...
   */
  async editMessage(message, body = null, metadata = null, asset = null) {
    if (body) {
      message.body = body;
    }
    if (metadata) {
  ...
  async deleteMessage(message) {
    const data = await skygear.lambda('chat:delete_message', [message._id]);
   ...
   */
  async getMessages(conversation, limit = 50, beforeTime = null, order = null) {
  ...
  async getMessageReceipts(message) {
    const messageID = message._id;
    const data = await skygear .lambda('chat:get_receipt', [messageID]);
   ...
   */
  async markAsDelivered(messages) {
   ...
   */
  async markAsLastMessageRead(conversation, message) {
    const uc = await this._getUserConversation(conversation);

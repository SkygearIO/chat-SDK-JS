...
export class SkygearChatContainer {
  ...
  /**
   * editMessage edit the body of an existing message
   *
   * @example
   * const skygearChat = require('skygear-chat');
   * skygearChat.editMessage(
   * 	'689bddac-656a-44ff-a87a-b279c6871257',
   *    'new message Body',
   * ).then(function(result) {
   *   console.log('Save success', result);
   * });
   ...
   * @param {string} newBody - new message body
   ...
   *    'b298ff59-ae68-45ab-bd04-67095fd0221a'
   * ).then(function(result) {
  ...
  deleteMessage(messageID) {
    return skygear
      .lambda('chat:delete_message', [messageID])
      .then(function (data) {

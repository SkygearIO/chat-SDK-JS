...
export class SkygearChatContainer {
  ...
  createConversation(participants, title = null, meta = {}, options = {}) {
    const conversation = new Conversation(participants, title, meta, options);
    return skygear.publicDB.save(conversation.toRecord());
  }
  ...
  _getMessageIDsByUserConversations(userConversations) {
    return _.reduce(userConversations, function (mids, record) {
      let conversation = record.$transient.conversation;
      if (conversation.last_message) {
        const mid = skygear.Record.parseID(conversation.last_message.id)[1];
        mids.push(mid);
      }
      if (record.last_read_message) {
        const last_read_message_id = skygear.Record.parseID(
                record.last_read_message.id)[1];
  ...
  _getMessagesByIDs(messageIDs) {
    return skygear
      .lambda('chat:get_messages_by_ids', [messageIDs])
      .then(function (data) {
        return _.reduce(data.results, function (byID, m) {
          byID[m._id] = m;
          return byID;
        }, {});
      });
  ...
  _getConversationsWithMessages(records) {
    let messageIDs = this._getMessageIDsByUserConversations(records);
    return this._getMessagesByIDs(messageIDs)
        .then((messagesByID) => {
          return _.map(records,
            (userConversation) => {
              let last_message = null;
              let last_read_message = null;

              let last_message_ref_from_record =
                  userConversation.$transient.conversation.last_message;
              let last_read_message_ref_from_record =
                  userConversation.last_read_message;

              if (last_message_ref_from_record) {
                last_message =
                    messagesByID[last_message_ref_from_record._id];
              }

              if (last_read_message_ref_from_record) {
                last_read_message =
                    messagesByID[last_read_message_ref_from_record._id];
              }

              return Conversation.fromRecord(
                userConversation.$transient.conversation,
                userConversation.unread_count,
                last_message,
                last_read_message_ref_from_record,
                last_read_message);
            }
          );
        }
  ...
  getConversation(conversationID, includeLastMessage = true) {
    ...
    query.equalTo('user', skygear.auth.currentUser.id);
    query.equalTo('conversation',
            new skygear.Reference('conversation/' + conversationID));
    ...
    return skygear.publicDB.query(query).then((records) => {
      if (records.length > 0) {
        ...
        if (includeLastMessage) {
          return this._getConversationsWithMessages([uc]).then(
           (result) => {
             return result[0];
           });
        } else {
          return Conversation.fromRecord(uc.$transient.conversation,
                  uc.unread_count);
        }
      }
  ...
  getConversations(page = 1, pageSize = 50, includeLastMessage = true) {
    ...
    return skygear.publicDB.query(query).then((records) => {
      if (includeLastMessage) {
        return this._getConversationsWithMessages(records);
      }
      return _.map(records, (record) => {
        return Conversation.fromRecord(record.$transient.conversation,
                record.unread_count);
      });
    });
  }
